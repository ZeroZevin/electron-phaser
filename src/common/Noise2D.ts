 ...
 */
export class Perlin {
  private GeneratorNoiseX = 1619
  private GeneratorNoiseY = 31337
  private GeneratorNoiseZ = 6971
  private GeneratorSeed = 1013
  private GeneratorShift = 8
  private Randoms =
    [
  ...
  private _quality = QualityMode.Medium
  private _octaveCount = 6
  private _persistence = 0.5
  private _seed = 0

  constructor (seed: number, quality = QualityMode.Medium) {
    this._seed = seed || 0
  ...
  GetValue (x: number, y: number, z: number) {
    ...
    for (let i = 0; i < this._octaveCount; i++) {
      let nx = this.MakeInt32Range(x)
      let ny = this.MakeInt32Range(y)
      let nz = this.MakeInt32Range(z)
      let seed = (this._seed + i) & 0xffffffff
      let signal = this.GradientCoherentNoise3D(nx, ny, nz, seed, this._quality)
      value += signal * cp
      x *= this._lacunarity
      y *= this._lacunarity
      z *= this._lacunarity
      cp *= this._persistence
    }
    return value
  }

  MakeInt32Range (value: number) {
    if (value >= 1073741824.0) {
      return (2.0 * this.IEEERemainder(value, 1073741824.0)) - 1073741824.0
    }
    if (value <= -1073741824.0) {
      return (2.0 * this.IEEERemainder(value, 1073741824.0)) + 1073741824.0
    }
  ...
  GradientCoherentNoise3D (x: number, y: number, z: number, seed: number, quality: QualityMode) {
    let x0 = x > 0.0 ? Math.floor(x) : Math.floor(x) - 1
    let x1 = x0 + 1
    let y0 = y > 0.0 ? Math.floor(y) : Math.floor(y) - 1
    let y1 = y0 + 1
    let z0 = z > 0.0 ? Math.floor(z) : Math.floor(z) - 1
    let z1 = z0 + 1
    let xs = 0
    let ys = 0
    let zs = 0
    switch (quality) {
      ...
      case QualityMode.Medium:
        {
          xs = this.MapCubicSCurve(x - x0)
          ys = this.MapCubicSCurve(y - y0)
          zs = this.MapCubicSCurve(z - z0)
          break
        }
      case QualityMode.High:
        {
          xs = this.MapQuinticSCurve(x - x0)
          ys = this.MapQuinticSCurve(y - y0)
          zs = this.MapQuinticSCurve(z - z0)
          break
        }
    }
    let n0 = this.GradientNoise3D(x, y, z, x0, y0, z0, seed)
    let n1 = this.GradientNoise3D(x, y, z, x1, y0, z0, seed)
    let ix0 = this.InterpolateLinear(n0, n1, xs)
    n0 = this.GradientNoise3D(x, y, z, x0, y1, z0, seed)
    n1 = this.GradientNoise3D(x, y, z, x1, y1, z0, seed)
    let ix1 = this.InterpolateLinear(n0, n1, xs)
    let iy0 = this.InterpolateLinear(ix0, ix1, ys)
    n0 = this.GradientNoise3D(x, y, z, x0, y0, z1, seed)
    n1 = this.GradientNoise3D(x, y, z, x1, y0, z1, seed)
    ix0 = this.InterpolateLinear(n0, n1, xs)
    n0 = this.GradientNoise3D(x, y, z, x0, y1, z1, seed)
    n1 = this.GradientNoise3D(x, y, z, x1, y1, z1, seed)
    ix1 = this.InterpolateLinear(n0, n1, xs)
    let iy1 = this.InterpolateLinear(ix0, ix1, ys)
    return this.InterpolateLinear(iy0, iy1, zs)
  }

  MapCubicSCurve (value: number) {
    return (value * value * (3.0 - 2.0 * value))
  }

  MapQuinticSCurve (value: number) {
    let a3 = value * value * value
    let a4 = a3 * value
    let a5 = a4 * value
  ...
  GradientNoise3D (fx: number, fy: number, fz: number, ix: number, iy: number, iz: number, seed: number) {
    ...
    let yvg = this.Randoms[(i << 2) + 1]
    let zvg = this.Randoms[(i << 2) + 2]
    let xvp = (fx - ix)
    let yvp = (fy - iy)
    let zvp = (fz - iz)
  ...
  InterpolateLinear (a: number, b: number, position: number) {
    return ((1.0 - position) * a) + (position * b)
  }

  IEEERemainder (dividend: number, divisor: number) {
    if (divisor === 0) {
      throw new Error('Zero of divisor')
    }
 ...
 * 2D 地形生成类
 * usage:
 * var pNose = new Perlin(); var noiseMap = new Noise2D(256, 256, pNose);
 * noiseMap.GeneratePlanar(offsetX * 2, offsetX * 2 + 2, offsetY * 2, offsetY * 2 + 2);
 * noiseMap.get(i,j, false)
 */
export class Noise2D {
  ...
  constructor (width: number, height: number, generator: any = null) {
    ...
    this._ucData = []
    for (let i = 0; i < width + this._ucBorder * 2; i++) {
      this._ucData.push([])
      for (let j = 0; j < height + this._ucBorder * 2; j++) {
        this._ucData[i].push(0)
      }
    }
  }

  GeneratePlanar_ (x: number, y: number) {
    return this._generator.GetValue(x, 0.0, y)
  }

  GeneratePlanar (left: number, right: number, top: number, bottom: number, isSeamless = true) {
    ...
    let xe = right - left
    let ze = bottom - top
    let xd = xe / (this._width - this._ucBorder)
    let zd = ze / (this._height - this._ucBorder)
    let xc = left

    for (let x = 0; x < this._ucWidth; x++) {
      let zc = top
      for (let y = 0; y < this._ucHeight; y++) {
        let fv
        if (isSeamless) {
          fv = this.GeneratePlanar_(xc, zc)
        } else {
          let swv = this.GeneratePlanar_(xc, zc)
          let sev = this.GeneratePlanar_(xc + xe, zc)
          let nwv = this.GeneratePlanar_(xc, zc + ze)
          let nev = this.GeneratePlanar_(xc + xe, zc + ze)
          let xb = 1.0 - ((xc - left) / xe)
          let zb = 1.0 - ((zc - top) / ze)
          let z0 = this.InterpolateLinear(swv, sev, xb)
          let z1 = this.InterpolateLinear(nwv, nev, xb)
          fv = this.InterpolateLinear(z0, z1, zb)
        }
        this._ucData[x][y] = fv
        if (x >= this._ucBorder && y >= this._ucBorder && x < this._width + this._ucBorder &&
          y < this._height + this._ucBorder) {
          this._data[x - this._ucBorder][y - this._ucBorder] = fv // Cropped data
        }
        zc += zd
      }
      xc += xd
  ...
  get (x: number, y: number, isCropped = true) {
    if (isCropped) {
