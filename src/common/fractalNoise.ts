...
export class FractalNoise {
  ...
  makeRectangle (
    ...
    const { amplitude, frequency, octaves, persistence } = this.processOptions(options)
    ...
    for (let x = 0; x < width; x++) {
      ...
      for (let y = 0; y < height; y++) {
        ...
        for (let octave = 0; octave < octaves; octave++) {
          ...
          value += this.simplex2(x * freq, y * freq) * (amplitude * Math.pow(persistence, octave))
        }
  ...
  processOptions (options: Options): Options {
    return {
